...
class WandPotionEffect(effect: String, capacity:java.lang.Integer, drawRF: java.lang.Integer) extends WandActivable(capacity, drawRF) {
    ...
    if (isActive(stack)){
      entityIn match {
        case player: EntityPlayer =>
          player.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("minecraft:" + effect), 5, 5))
        case _ =>
      }
    }
